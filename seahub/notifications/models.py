...
class UserNotification(models.Model):
    ...
    def format_file_comment_msg(self):
        ...
        repo = seafile_api.get_repo(repo_id)
        if repo is None or not seafile_api.get_file_id_by_path(file_path):
            self.delete()
            return None

        file_name = os.path.basename(file_path)
        msg = _("file <a href='%(file_url)'>%(file_name)s</a> has a new comment from user %(author)") % {
            'file_url': reverse('view_lib_file', args=[repo_id, file_path]),
            'file_name': file_name,
            'author': email2nickname(author),
        }
        return msg
